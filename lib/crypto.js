 ...
 * using ID-based deterministic keys.
 */
...
/**
 * Derives a stable CryptoKey from two user IDs.
 */
export async function getSharedKey(uid1, uid2) {
    if (typeof window === 'undefined' || !window.crypto || !window.crypto.subtle) {
        console.warn('Encryption unavailable: Web Crypto API requires a secure context (HTTPS)');
        return null;
    }

    // Stable sort so the ID order doesn't matter
    const sortedIds = [uid1, uid2].sort().join(':');
    const encoder = new TextEncoder();
    const data = encoder.encode(sortedIds);

    // Hash the IDs to get a fixed-length 256-bit entropy
